    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(dataloader)):
            ...
            if (i + 1) % opt.eval_print_freq == 0:
                logger.warning(
                    "Hits1: %d, %d / %d -- average = %.3f" % (opt.global_rank, i + 1, len(dataloader), np.mean(ems))
                )
                logger.warning(
                    "F1: %d, %d / %d -- average = %.3f" % (opt.global_rank, i + 1, len(dataloader), np.mean(f1s))#
                )

    logger.warning("Hits1: %d, total %d -- average = %.3f" % (opt.global_rank, total, np.mean(ems)))
    logger.warning("F1: %d, total %d -- average = %.3f" % (opt.global_rank, total, np.mean(f1s)))#
    if opt.world_size > 1 and not opt.local_rank == -1:
        torch.distributed.barrier()
    score, total = util.weighted_average(np.mean(ems), total, opt)
    score_f1,total=util.weighted_average(np.mean(f1s), total, opt)#
...
if __name__ == "__main__":
    ...
    ems,f1s = evaluate(model, test_dataset, test_dataloader, tokenizer, opt)

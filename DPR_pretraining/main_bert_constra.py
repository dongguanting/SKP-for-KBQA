...
def main():
    ...
    parser.add_argument('--train_data', default="/mnt/dolphinfs/hdd_pool/docker/user/hadoop-aipnlp/dongguanting/bert_pretraining/data/random_10w.txt", help='train_data dir')
    ...
    train_ids = TensorDataset(train_origin_ids,train_origin_ids)   
    ...
    if args.do_train:
        ...
        for epoch in range(args.epoch):
            ...
            for step, data in enumerate(train_loader, 0): #这里的data就是train_loader的每一个元组（mask的输入embedding，原文输入embedding）
                ...
                # origin_logits = dropout(logits)

                #正样本
                dropout = nn.Dropout(0.1)
                aug_logits = dropout(origin_logits)
                
                
                
                loss = instance_CL_Loss(origin_logits, aug_logits, 'origin', 0.5)
                ...
                if step % 20 == 19:   #打印每2000步mini batch
                    ...
                    running_loss = 0.0
            count = 0
            torch.save(model.state_dict(),os.path.join(args.model_save_dir,f'constra_model_{epoch}.bin'))

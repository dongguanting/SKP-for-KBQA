...
def gen_ctx_vectors(
    ...
    for j, batch_start in enumerate(range(0, n, bsz)):
        ...
        batch_token_tensors = [
            tensorizer.text_to_tensor(ctx[1], None) for ctx in batch
        ]
        ...
        # ctx_seg_batch = move_to_device(torch.zeros(ctx_ids_batch.size(),dtype=ctx_ids_batch.dtype,layout=ctx_ids_batch.layout, device=ctx_ids_batch.device), cfg.device)
...
def main(cfg: DictConfig):
    ...
    # import pdb
    # pdb.set_trace()
    all_passages = [ln.split("\t") for ln in open(cfg.ctx_src)]
    all_passages = all_passages[1:]

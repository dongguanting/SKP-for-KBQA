...
from dpr.models.biencoder_mask_matrix import BiEncoderNllLoss, BiEncoderBatch
...
'''DGT MASK ATTention'''
def mask_mask(context_ids,tensorizer):
    bsz=len(context_ids)
    record=[[] for i in range(bsz)]
    for i,sample in enumerate(context_ids):
        sample = tensorizer.tokenizer.convert_ids_to_tokens(sample)
        # import pdb
        # pdb.set_trace()
    ...
    context_mask=np.full((bsz,256,256), False)
...
def _do_biencoder_fwd_pass(
    ...
    # import pdb
    # pdb.set_trace()
    ctx_attn_mask=mask_mask(input.context_ids,tensorizer)
    ctx_attn_mask=torch.Tensor(ctx_attn_mask).cuda()
    '''DGT_MASK'''
